...
house_xls = common.get_xls("houseCase.xlsx", "tujia_getPrice")
...
class Getprice(unittest.TestCase):
    def setParameters(self, case_name, method, token, room_id, check_in, check_out, currency, guest_count, quantity, result, code, msg):
        """
        set params
        :param case_name:
        :param method:
    ...
    def testGetprice(self):
        ...
        """
        # set url
        self.url = common.get_url_from_xml1('getPrice')
    ...
    def tearDown(self):
        ...
        if info['result_code'] == 0:
            ...
            price_rb = common.get_group_from_return_json(info, 'prices')
            # set user token to config file
            localReadConfig.set_order("prices", str(price_rb))
        else:
            pass
        self.log.build_case_line(self.case_name, str(self.info['result_code']), str(self.info['prices']))
        print("测试结束，输出log完结\n\n")
